...
void GetSQLInfo(char* server, char* database, WCHAR** dispatch) {
	SQLHENV env = NULL;
	SQLHSTMT stmt = NULL;
	SQLRETURN ret;
	SQLINFO info;
	...
	SQLHDBC dbc = ConnectToSqlServer(&env, server, database, dispatch);
	...
	BadgerDispatch(dispatch, "%s %s\n", "[*] ExecuteQuery Aufruf mit:", query);
	if (!ExecuteQuery(stmt, query))
	{
		goto END;
		BadgerDispatch(dispatch, "%s\n", "[*] gotoENd");
	}
	BadgerDispatch(dispatch, "%s\n", "[*] GetSingleResult Aufruf mit");
	char* result = GetSingleResult(stmt, FALSE);
	BadgerDispatch(dispatch, "%s %s\n", "[*] GetSingleResult Ergebnis:", result);
...
END:
	FreeSqlInfo(&info);
	ODBC32$SQLCloseCursor(stmt);
	DisconnectSqlServer(env, dbc, stmt, dispatch);
...
void coffee(char** argv, int argc, WCHAR** dispatch)
{
    ...
    server = "localhost";
    database = "master";
    GetSQLInfo(server, database, dispatch);
